...
describe("planToolPurchases", () => {
  ...
  it("treats a max in inventory of 0 as unlimited", () => {
    const state: GameState = {
      ...GAME_STATE,
      inventory: {
        ...GAME_STATE.inventory,
      ...
      settings: {
        ...GAME_STATE.settings,
        toolShop: { buyAll: { Axe: { maxInInventory: 0 } } },
      },
    };
  ...
  it("caps the purchase amount at the player's configured max in inventory", () => {
    const state: GameState = {
      ...GAME_STATE,
      inventory: {
        ...GAME_STATE.inventory,
      ...
      settings: {
        ...GAME_STATE.settings,
        toolShop: { buyAll: { Axe: { maxInInventory: 5 } } },
      },
    };
  ...
  it("skips a tool already at or above its configured max in inventory", () => {
    const state: GameState = {
      ...GAME_STATE,
      inventory: {
        ...GAME_STATE.inventory,
